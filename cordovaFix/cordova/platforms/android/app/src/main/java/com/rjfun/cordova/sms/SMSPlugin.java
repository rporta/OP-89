    ...
    private PluginResult deleteSMS(JSONObject filter, CallbackContext callbackContext) {
        ...
        try {
            ...
            while (cur.moveToNext()) {
                ...
                LOG.d(LOGTAG, nameofCurrMethod +
                        ", intento de borrado de sms : " +  id
                );

                // ramiro portas,
                // no se van a borrar los sms, se va a actualizar a read 1

                ContentValues values = new ContentValues();
                values.put("read", true);

                // ctx.getContentResolver().update(Uri.parse("content://sms/inbox"), values, "_id=" + 360,(String[])null);
            ...
            LOG.d(LOGTAG, nameofCurrMethod +
                    ", intento de borrado de sms, falló : " +  e
            );
        }
        return null;
    }
